...
export const tokinizer = (action: ActionEnum): { access_token: string, refresh_token: string } => {
  ...
  switch (action) {
    ...
    case ActionEnum.CASHIER_REGISTER:
    ...
    case ActionEnum.FORGOT_PASSWORD:
      ...
      break;
    case ActionEnum.CASH_REGISTER:
      access_token = jwt.sign({}, config.JWT_CONFIRM_CASH_SECRET, {expiresIn: config.JWT_CONFIRM_CASH_LIFETIME});
      break;
